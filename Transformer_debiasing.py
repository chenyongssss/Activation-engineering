...
from models.transformer import PS_Transformer, PS_TransformerDebias

###########################################
# 1. Functions for fitting the null distribution and computing systematic error
###########################################

NUM_CONTINUOUS = 0  # TODO Number of continuous variables, need to move it to the configuration later instead of global
...
def get_preds(batch_x, network):
        """
        Get the predictions from list of batches from each modality
        :param batch_xs:
        :return:
        """
        ...
        if MODEL_TYPE == "transformer":
            ...
            ncon = NUM_CONTINUOUS
            if ncon == 0:
                out = network(batch_x[:, :], batch_x[:, 0:0])
            else:
                out = m(batch_x[:, :-ncon], batch_x[:, -ncon:])
...
def compute_bias_vectors(model, X_biased, X_neutral, layer_indices=None):
    ...
    with torch.no_grad():
        # TODO Only use the latent features after the transformer for now, can dig into the transformer later
        if ncon == 0:
            a_biased = model.transformer(X_biased[:, :], X_biased[:, 0:0])
            a_neutral = model.transformer(X_neutral[:, :], X_neutral[:, 0:0])
        else:
            a_biased = model.transformer(X_biased[:, :-ncon], X_biased[:, -ncon:])
            a_neutral = model.transformer(X_neutral[:, :-ncon], X_neutral[:, -ncon:])
...
def train_NCO(nco, Xt, Wt, weight_1, weight_0):
    ...
    nco.eval()   
    with torch.no_grad():   
        test_outputs = get_preds(Xt, nco).squeeze()
...
def test_NCO(nco, Xt, Wt):
    nco.eval()  
    with torch.no_grad():
        NCO_pred = get_preds(Xt, nco).squeeze().numpy()
...
if __name__ == "__main__":
    ...
    model = PS_Transformer(model_config)
    ...
    model.eval()   
    with torch.no_grad():
        test_outputs = get_preds(Xt, model).squeeze()
    ...
    with torch.no_grad():
        ...
        ps = get_preds(torch.tensor(X, dtype=torch.float32), model).squeeze().numpy()
    wt = np.ones(ps.shape[0])
    wt[A==1] = 1/ps[A==1]
    wt[A==0] = 1/(1-ps[A==0])
